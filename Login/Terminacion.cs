...
namespace Login
{
    public partial class Terminacion : Form
    {
        public Terminacion()
        {
            InitializeComponent();
        ...
        private void Terminacion_Load(object sender, EventArgs e)
        {
            ...
            {
                //completo el cuadro
                DataTable dt2 = Conexion.GetTerminacion();
                Cuadro.Rows.Clear();
                if (dt2 != null)
                {
        ...
        private void Agregar_Click(object sender, EventArgs e)
        {
            if ( descripcionTBX.Text != "" )
            {
                try
                {
                   Conexion.AddTerminacion(descripcionTBX.Text);
                    Limpiar();
                    Terminacion_Load(sender, e);
                }
                
                catch (Exception) { MessageBox.Show("No se pudo agregar la Terminacion, revise los datos y reintente"); }
            }
        ...
        private void ModificarBTN_Click(object sender, EventArgs e)
        {
            if (descripcionTBX.Text != "")
            {
                try
                {
                    ...
                    if (check == true)
                    {
                        Conexion.SetTerminacion((int)Cuadro.Rows[n].Cells[1].Value,descripcionTBX.Text);
                        Limpiar();
                        Terminacion_Load(sender, e);
                    }
                ...
                catch (Exception) { MessageBox.Show("No se pudo modificar la Terminacion, revise los datos y reintente"); }
            }
            else { MessageBox.Show("Revise los campos ingresados"); }
        }
        ...
        private void EliminarBTN_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (check == true)
                {
                   Conexion.DeleteTerminacion((int)Cuadro.Rows[n].Cells[1].Value);
                    Limpiar();
                    Terminacion_Load(sender, e);
                }
