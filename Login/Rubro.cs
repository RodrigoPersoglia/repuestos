...
namespace Login
{
    public partial class Rubro : Form
    {
        ...
        private void Temple_Load(object sender, EventArgs e)
        {
            ...
            {
                //completo el cuadro
                DataTable dt2 = Conexion.GetTemple();
                Cuadro.Rows.Clear();
                if (dt2 != null)
                {
        ...
        private void Agregar_Click(object sender, EventArgs e)
        {
            if ( descripcionTBX.Text != "" )
            {
                try
                {
                   Conexion.AddTemple(descripcionTBX.Text);
                    Limpiar();
                    Temple_Load(sender, e);
                }
                
                catch (Exception) { MessageBox.Show("No se pudo agregar la Temple, revise los datos y reintente"); }
            }
        ...
        private void ModificarBTN_Click(object sender, EventArgs e)
        {
            if (descripcionTBX.Text != "")
            {
                try
                {
                    ...
                    if (check == true)
                    {
                        Conexion.SetTemple((int)Cuadro.Rows[n].Cells[1].Value,descripcionTBX.Text);
                        Limpiar();
                        Temple_Load(sender, e);
                    }
                ...
                catch (Exception) { MessageBox.Show("No se pudo modificar la Temple, revise los datos y reintente"); }
            }
            else { MessageBox.Show("Revise los campos ingresados"); }
        }
        ...
        private void EliminarBTN_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (check == true)
                {
                   Conexion.DeleteTemple((int)Cuadro.Rows[n].Cells[1].Value);
                    Limpiar();
                    Temple_Load(sender, e);
                }
