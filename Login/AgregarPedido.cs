...
namespace Login
{
	public partial class AgregarPedido : Form
	{
		...
		public AgregarPedido() { InitializeComponent(); } // Constructor


		void AgregarPedidoLoad(object sender, EventArgs e) // Carga los Combobox iniciales desde la base de datos. EVENTO LOAD
		{
			...
			// TEXTBOX NUMERO PEDIDO
			string consulta8 = "select max(p.numero) from pedido p limit 1";
			int numero = 0;
			try {
				...
				if (reader.HasRows) {
					while (reader.Read()) 
					{
						try 
						{
							numero = int.Parse(reader.GetString(0)); 
						}
                        catch (Exception) { }
					}
				}
		...
		void BuscarImagenClick(object sender, EventArgs e) // Busca la Imagen y la muestra en el pictureBox
		{
			OpenFileDialog buscar = new OpenFileDialog();
			if (buscar.ShowDialog() == DialogResult.OK) {
				OCTxt.Text = buscar.FileName;
				pictureBox1.ImageLocation = OCTxt.Text;
			}
		...
		void CrearClick(object sender, EventArgs e) // Guarda el pedido en la base de datos
		{ 
			try {
				...
				int _estadoID = 1; // El estado del pedido siempre se crea en 1.
				...
				if (dataGridView1.Rows.Count>1 && _fecha != null && AleacionComboBox.Text != "Seleccione" && UnidadComboBox.Text != "Seleccione" && TerminacionComboBox.Text != "Seleccione" && PrioridadComboBox.Text != "Seleccione" && ClienteComboBox.Text != "Seleccione" && _articuloID != 0 && TemplecomboBox.Text != "Seleccione") {
					//Desterminamos el string para hacer la insercion en la base de datos.
					string sql = "INSERT INTO pedido (numero,fecha,kgEstimados,OC,Observaciones,Cliente_ID,Articulo_ID,Unidad_ID,Terminacion_ID,Estado_ID,Prioridad_ID,Aleacion_ID,Temple_ID,Matriz_ID) VALUES ('" + _numero + "', '" + _fecha + "', '" + kgsEstimados + "','" + _oc + "','" + _observaciones + "','" + _clienteID + "','" + _articuloID + "','" + _unidadID + "','" + _terminacionID + "','" + _estadoID + "','" + _PrioridadID + "','" + _aleacionID + "','" + _templeID + "','" + matriz.ID + "')";
					...
					try
					{
						...
						// Obtenemos el ID del pedido guardado
						string sql2 = "select ID from pedido p where p.numero = " + NumeroTxt.Text;
						int numeroID = 0;
						try
						{
							MySqlDataReader reader2;
							MySqlCommand comand = new MySqlCommand(sql2, conexionBD);
							reader2 = comand.ExecuteReader();
							if (reader2.HasRows)
							{
								while (reader2.Read()) { numeroID = int.Parse(reader2.GetString(0)); }
							}
						}
						catch (MySqlException ex) { MessageBox.Show("Error al buscar " + ex.Message,"Atención",MessageBoxButtons.OK,MessageBoxIcon.Error); }

						//generamos una nueva Conexion y escribimos los datos del cuadro en PedidoDetalle
						...
						try
						{
							...
							{
								string sql3 = "Insert into pedidoDetalle (cantidad,largo,Pedido_ID,kgEstimados) VALUES ('" + dataGridView1.Rows[fila].Cells[0].Value + "', '" + dataGridView1.Rows[fila].Cells[1].Value + "', '" + numeroID + "', '" + dataGridView1.Rows[fila].Cells[2].Value + "')";
								MySqlCommand comando2 = new MySqlCommand(sql3, conexionBD2);
								comando2.ExecuteNonQuery();
							}
						}
						catch (Exception ex) { MessageBox.Show("error pedido detalle: " + ex.Message,"Atención",MessageBoxButtons.OK,MessageBoxIcon.Error); }
						...
						if (checkImprimir.Checked == true)
						{
							...
							catch (Exception) { MessageBox.Show("No se pudo imprimir el pedido", "Atención", MessageBoxButtons.OK, MessageBoxIcon.Error); }
						...
						//Para estableces el siguiente numero de pedido hacemos una nueva consulta, obtenemos el ultimo pedido y le sumamos 1.
						int numero = 0;
						MySqlDataReader reader;
						MySqlConnection conexionBD3 = Conexion.ObtenerConexion();
						conexionBD3.Open();
						string consulta8 = "select max(p.numero) from pedido p limit 1";
						try {
							MySqlCommand comand = new MySqlCommand(consulta8, conexionBD3);
							reader = comand.ExecuteReader();
							if (reader.HasRows) {
								while (reader.Read()) { numero = int.Parse(reader.GetString(0)); }
							}
						}
						catch (MySqlException ex) { MessageBox.Show("Error al buscar " + ex.Message,"Atención",MessageBoxButtons.OK,MessageBoxIcon.Error); } //ERROR ACA!!
                        finally { conexionBD3.Close(); }
						numero += 1; // Tomamos el valor que nos arroja la consulta y lo incrementamos en 1
						NumeroTxt.Text = numero.ToString(); // Mostramos el valor resultante que será nuestro nuevo numero de pedido

					}

					catch (MySqlException ex) { MessageBox.Show("Error al guardar: " + ex.Message, "Atención", MessageBoxButtons.OK, MessageBoxIcon.Error); }
			...
			catch (Exception fex) { MessageBox.Show("Datos incorrectos: " + fex.Message, "Atención", MessageBoxButtons.OK, MessageBoxIcon.Error); }
		}
		...
		{ 
			...
			catch (Exception) { MessageBox.Show("No se encontro una imagen", "Atención", MessageBoxButtons.OK, MessageBoxIcon.Warning); pictureBox1.Image = null; }
		}
		...
		private void imprimir2(object sender, PrintPageEventArgs e)
		{
			...
			if (UnidadComboBox.Text == "Tiras")
            {
				...
				for (int fila = 0; fila < dataGridView1.Rows.Count - 1; fila++)
				{
					...
					cant += decimal.ToDouble((decimal)dataGridView1.Rows[fila].Cells[0].Value);
				}
        ...
        private void BuscarImagen_Click(object sender, EventArgs e)
        {
			...
			{
				MessageBox.Show("El archivo seleccionado no es un tipo de imagen válido", "Atención", MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
		}
    }
}
