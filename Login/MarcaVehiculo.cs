...
namespace Login
{
    public partial class MarcaVehiculo : Form
    {
        ...
        private void Aleacion_Load(object sender, EventArgs e)
        {
            ...
            {
                //completo el cuadro
                DataTable dt2 = Conexion.GetMarcaVehiculo();
                Cuadro.Rows.Clear();
                if (dt2 != null)
                {
        ...
        private void Agregar_Click(object sender, EventArgs e)
        {
            if ( descripcionTBX.Text != "" )
            {
                try
                {
                    Conexion.AddMarcaVehiculo(descripcionTBX.Text);
                    Limpiar();
                    Aleacion_Load(sender, e);
                }
                
                catch (Exception) { MessageBox.Show("No se pudo agregar la MarcaVehiculo, revise los datos y reintente"); }
            }
        ...
        private void ModificarBTN_Click(object sender, EventArgs e)
        {
            if (descripcionTBX.Text != "")
            {
                try
                {
                    ...
                    if (check == true)
                    {
                        Conexion.SetMarcaVehiculo((int)Cuadro.Rows[n].Cells[1].Value, descripcionTBX.Text);
                        Limpiar();
                        Aleacion_Load(sender, e);
                    }
                ...
                catch (Exception) { MessageBox.Show("No se pudo modificar la MarcaVehiculo, revise los datos y reintente"); }
            }
            else { MessageBox.Show("Revise los campos ingresados"); }
        }
        ...
        private void EliminarBTN_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (check == true)
                {
                    Conexion.DeleteMarcaVehiculo((int)Cuadro.Rows[n].Cells[1].Value);
                    Limpiar();
                    Aleacion_Load(sender, e);
                }
            ...
            catch (Exception) { MessageBox.Show("No se pudo eliminar la MarcaVehiculo, revise los datos y reintente"); }
        }
