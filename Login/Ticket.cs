...
namespace Login
{
    ...
    public class CreaTicket
    {
        ...
        public static void imprimir(Comprobante comprobante, DataTable dt)
        {
            ...
            Ticket1.TextoExtremos("Fecha: " + comprobante.FechaHora.ToString("dd/MM/yyyy"), "Hora: " + comprobante.FechaHora.ToString("HH:mm:ss"));
            Ticket1.TextoDerecha("Comprobante Nro: " + comprobante.Numero.ToString().PadLeft(8, '0'));
            Ticket1.TextoIzquierda("Cliente: " + comprobante.Cliente);
            Ticket1.TextoIzquierda(comprobante.Direccion + " - " + comprobante.Localidad);
            //Ticket1.TextoCentro(""); // imprime en el centro "Venta mostrador"
            Ticket1.LineasIgual();
            ...
            {
                int cantidad = int.Parse(x[3].ToString());
                double precio = Math.Round(double.Parse(x[4].ToString()), 2);
                double total2 = Math.Round(double.Parse(x[5].ToString()), 2);
                Ticket1.TextoIzquierda((string)x[2]);
                Ticket1.TextoExtremos(cantidad.ToString() + "   x   $" + precio.ToString(), "$" + total2.ToString());
                //Ticket1.AgregaArticulo((string)x[2],cantidad ,precio ,total2 ); //imprime una linea de descripcion

            }
            Ticket1.TextoCentro("");
            Ticket1.TextoDerecha("Sub-Total:" + ("$" + comprobante.SubTotal.ToString()).PadLeft(10, ' '));
            Ticket1.TextoDerecha("Recargo Financiero:" + ("$" + comprobante.Financiación.ToString()).PadLeft(10, ' '));
            Ticket1.TextoDerecha("TOTAL:" + ("$" + comprobante.Total.ToString()).PadLeft(10, ' '));
            Ticket1.TextoCentro(""); // imprime en el centro "Venta mostrador"
            Ticket1.LineasAsterisco();
            Ticket1.TextoCentro("Gracias por su compra"); // imprime en el centro "Venta mostrador"
            Ticket1.LineasAsterisco();
        ...
        public static void Reimprimir(Comprobante comprobante, DataTable dt)
        {
            ...
            foreach (DataRow x in dt.Rows)
            {
                int cantidad = int.Parse(x[3].ToString());
                double precio = Math.Round(double.Parse(x[4].ToString()), 2);
                double total2 = Math.Round(double.Parse(x[4].ToString())* int.Parse(x[3].ToString()), 2);
                Ticket1.TextoIzquierda((string)x[2]);
                Ticket1.TextoExtremos(cantidad.ToString() + "   x   $" + precio.ToString(), "$" + total2.ToString());
                //Ticket1.AgregaArticulo((string)x[2],cantidad ,precio ,total2 ); //imprime una linea de descripcion
