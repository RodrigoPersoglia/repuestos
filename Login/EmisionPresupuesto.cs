...
namespace Login
{
	...
	/// </summary>
	public partial class EmisionPresupuesto : Form
	{
		//variables
		...
		//Constructor
		public EmisionPresupuesto( Usuario user)
		{
			usuario = user;
			InitializeComponent(); }
				...
				switch (result)
				{
					case DialogResult.Yes:
						if (Conexion.Validar(UsuarioCBX.Text))
						{
							...
							Conexion.AgregarPresupuesto(SubTotalNUM.Value, TotalRecargo.Value, TotalNUM.Value, cliente.ID, Cliente.Text, Direccion.Text, Localidad.Text, cliente.CP, "Rodrigo", Convert_to_DT(Cuadro),(int)MediosPagoCBX.SelectedValue);
                            if (ImprimeChek.Checked)
                            {

								comprobante = Conexion.GetUltimoPresupuesto();
								ImprimirPresupuesto(sender, e);
								//ImprimirDuplicadoPresupuesto(sender, e);
							}
		...
		private void ImprimirPresupuesto(object sender, EventArgs e)
		{
		...
		private void ImprimirDuplicadoPresupuesto(object sender, EventArgs e)
		{
			printDocument1 = new PrintDocument();
			PrinterSettings ps = new PrinterSettings();
			PaperSize A4 = new PaperSize();
		...
		private void imprimir(object sender, PrintPageEventArgs e)
		{
			...
			e.Graphics.DrawString(" Presupuesto n°: "+comprobante.Numero.ToString().PadLeft(8, '0'), Arial12, Brushes.Black, 565, 74);
