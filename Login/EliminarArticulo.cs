...
namespace Login
{
	public partial class EliminarArticulo : Form
	{
		...
		void EliminarArticuloLoad(object sender, EventArgs e)
		{
		...
		void CrearClick(object sender, EventArgs e)
		{
            ...
            try
            {
                if (IDTex.Text != "")
				{
					string sql = "DELETE FROM articulo  WHERE ID = " + IDTex.Text + ";";
                ...
                else { MessageBox.Show("Seleccione un árticulo válido"); }

        }
			
			catch (MySqlException){MessageBox.Show("No se puede eliminar un articulo que registre operaciones ");}
